...
- `npm run smoke:prod` verifies protected API behavior both ways: without `TRUSTACCEPT_SMOKE_SESSION_TOKEN`, `/api/v1/approvals`, `/api/v1/policy`, `/api/v1/approvals/evaluate`, and `/api/v1/approvals/by-run/[runId]` must return JSON `401`; with a token, approvals, policy, advisory evaluate, and run-rollup endpoints must return authenticated responses before optional approval creation runs. The advisory evaluate smoke expects the default fail-safe `require_human` decision and a `suggested_request_approval_args` payload.
